...
cd build/ 
cmake ../cmake/ -DCMAKE_BUILD_TYPE=Release  -DPKG_KSPACE=on -DPKG_MANYBODY=on -DPKG_RIGID=on -DPKG_MISC=on -DPKG_MOLECULE=on\
	-DBUILD_MPI=on -DPKG_GRANULAR=on -DMPI_C_COMPILER=mpicc -DMPI_CXX_COMPILER=mpic++ \
	-DBUILD_OMP=on || fail "Error when configuring LAMMPS build directory" 

make -j 4 || fail "Error when building LAMMPS"
cd ..
...
# build utils
#docker run --rm --user=$(id -u):$(id -g) -v $(pwd):$(pwd) -w $(pwd) ubuntu:mpi-dev make